...
For the model building and evaulation, we used the logistic regression function to analyze the relation between CVD and other risk factors. The regression model indicated that age, blood pressure, and the level of cholesterol is correlated with CVD. It is worth mentioning that the higher cholesterol level is strongly associated with increased the risk of CVD. Furthermore, the vif evaulatoin suggested that the variables have no multicollinearity, the Hosmer and Lemeshow test also showed that the model is a good fit.

We further used Rigged and Lasso regression in our model to ensure that the model is not overfitting. Furthermore, KNN classification analysis indicated that we only need 7-nearest neighbors to achieve 70% accuracy. For decision tree analysis, the result showed that blood pressure, age, and cholesterol level are important indicator to determine the CVD. This result is consist with the logistic regression analysis which showed that blood pressure and cholesterol level are correlated with CVD. 

Overall, this CVD database provides a useful variables for us to conduct sereval chi-square test, regression analysis, and model building. 
