...
### 5.1 Preparation: Standardize the numerical variables
...
```{r}
CardioRidge <- subset(cardio, select = -c(height, weight, prob))
CardioRidge = uzscale(CardioRidge)

x=model.matrix(cardio~.,CardioRidge)[,-2]
...
y_train = train %>% select(cardio) %>% unlist() # %>% as.numeric()
y_test = test %>% select(cardio) %>% unlist() # %>% as.numeric()
...
The best λ for Ridge regression is almost 0, which gives the least square fit and means all the coefficiences are used.
...
The best λ for Lasso regression is almost 0, which gives the least square fit and means all the coefficiences are used. This results agree with Ridge Regression.
